...
function giornalismo_register_required_plugins() {
    $plugins = array(
        ...
        array(
            'name'      => 'Giornalismo Story Details',
            'slug'      => 'giornalismo-story-details',
            'required'  => false,
        ),
		array(
			'name'      => 'Category Color',
...
if (!function_exists('giornalismo_todays_date')) {
	function giornalismo_todays_date() {
		if ( esc_attr( get_theme_mod( 'giornalismo-todays-date' ) ) ) {
			return '<div class="todays-date"><h3>' . date_i18n( "l, F j, Y" ) . '</h3></div>';
		}
	}
}
...
function giornalismo_archive_title($title) {
  if (is_day()) {
    $title = __('Archives for ', 'giornalismo') . get_the_time('F j, Y');
  }
  else if (is_month()) {
...
if (!function_exists('giornalismo_author_social_area')) {
	function giornalismo_author_social_area() {
		...
		if ( esc_url( get_the_author_meta( 'email' ) ) ) {
			$html .= '<div class="author-social"><a href="mailto:' . esc_url( get_the_author_meta( 'email' ) ) . '"><img src="' . get_template_directory_uri() . '/images/email.png" /></a></div>';
		}
		$html .= '</div>';
...
if (!function_exists('giornalismo_staff_author_social_area')) {
	function giornalismo_staff_author_social_area( $id ) {
		...
		if ( get_the_author_meta( 'email', $id ) ) {
			$html .= '<div class="author-social"><a href="mailto:' . esc_url( get_the_author_meta( 'email', $id ) ) . '"><img src="' . get_template_directory_uri() . '/images/email.png" /></a></div>';
		}
		$html .= '</div>';

...
<div id="container" class="clearfix">
  <div class="wrapper container_24 clearfix">
    <div class="page-title"> <a href="<?php the_permalink(); ?>" title="<?php single_post_title();  ?>">
      <?php if(is_page()){ single_post_title(); } else{ _e('Blog','irex-lite');} ?>
      </a> <i class="edit_link">
		...
		<?php if(has_post_thumbnail()) { ?>
         ...
         <?php } ?>
          ...
          <p class="entry-meta blog-entry-meta">
            ...
            <?php $key="ldc_plc"; if(get_post_meta($post->ID, $key, true)) { echo get_post_meta($post->ID, $key, true); } else { echo '0'; } ?>
            </span></span> <span class="comment"><?php echo $num_comments; ?></span> <span class="share"><span class='dislike_img'>
            <?php $key="ldc_pdc"; if(get_post_meta($post->ID, $key, true)) { echo get_post_meta($post->ID, $key, true); } else { echo '0'; } ?>
